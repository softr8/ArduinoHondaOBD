...
#
# PORT is auto-detected if omitted (first serial board arduino-cli sees).
...
# --- resolve port ---
PORT="${1:-$(arduino-cli board list | awk '/serial/ {print $1; exit}')}"
[ -n "$PORT" ] || die "no serial port found; pass one: tools/deploy-esp.sh /dev/cu.usbserial-XXXX"
say "Port: $PORT"
